        ...
        # encoder_1: [batch, 256, 256, in_channels] => [batch, 128, 128, ngf]
            with tf.device('/gpu:1'):
                with tf.variable_scope("encoder_1"):
                    output = conv2d(x, ngf, [11,1], [1,1,1,1])
                ...
                for out_channels in layer_specs:
                    with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
                        ...
                        output = conv2d(rectified, out_channels, [11,1], [1,1,2,1])
            ...
            with tf.device('/gpu:0'):
                ...
                for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
                    ...
                    with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
                        ...
                        # [batch, in_height, in_width, in_channels] => [batch, in_height*2, in_width*2, out_channels]
                        output = deconv_up(rectified, out_channels, [11,1], [1,1,1,1])
                ...
                with tf.variable_scope("decoder_1"):
                    ...
                    rectified = activation(input, 'prelu')
                    output = conv2d(rectified, ngf, [11,1], [1,1,1,1])
                    layers.append(output)

                with tf.variable_scope("output_layer"):
                    rectified = activation(layers[-1], 'prelu')
                    output = conv2d(rectified, 1, [3,1], [1,1,1,1])
                    layers.append(output)
        ...
        with tf.device('/gpu:0'):
            with tf.variable_scope(self.name) as vs:
                ...
                with tf.variable_scope("layer_1"):
                    convolved = conv2d(input, ndf, [3,1], [1,1,2,1])
                ...
                for i in range(n_layers):
                    with tf.variable_scope("layer_%d" % (len(layers) + 1)):
                        ...
                        convolved = conv2d(layers[-1], out_channels, [3,1], [1,1,stride,1])
                ...
                with tf.variable_scope("layer_%d" % (len(layers) + 1)):
                    convolved = conv2d(layers[-1], 1, [3,1], [1,1,1,1])
                    normalized = layernorm(convolved, axis=[1, 2, 3], name='D_layernorm')
                    rectified = activation(normalized, 'lrelu')
